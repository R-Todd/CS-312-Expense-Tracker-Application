// client/src/components/EditSavingsForm.js

// =========== IMPORTS ===========
import React, { useState } from 'react';
import DatePicker from 'react-datepicker';
import 'react-datepicker/dist/react-datepicker.css'; 
// styling
import '../App.css';
// ===============================

// EditSavingsForm component for updating an existing savings entry
const EditSavingsForm = ({ savingsEntry, onUpdate, onCancel }) => {

    // ==== State Variables ====
    const [formData, setFormData] = useState({
        amount: savingsEntry.amount,
        goal: savingsEntry.goal, 
        date: new Date(savingsEntry.date), 
        description: savingsEntry.description || ''
    });
    const [error, setError] = useState('');
    const { amount, goal, date, description } = formData;
    // =========================

    // ---- event handlers ----
    const onChange = (event) => {
        setFormData({ ...formData, [event.target.name]: event.target.value });
    };
    
    const handleDateChange = (newDate) => {
        setFormData({ ...formData, date: newDate });
    };

    const onSubmit = async (event) => {
        event.preventDefault();
        setError('');

        if (parseFloat(amount) <= 0) {
            setError('Amount must be positive');
            return;
        }

        try {
            const token = localStorage.getItem('token');
            if (!token) {
                setError('No token found, please log in');
                return;
            }

            // --- api call (PUT request) ---
            const response = await fetch(`/api/savings/${savingsEntry.savings_id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'x-auth-token': token
                },
                body: JSON.stringify({
                    amount: parseFloat(amount),
                    goal, 
                    date: date.toISOString().split('T')[0],
                    description
                })
            });
            
            if (!response.ok) {
                const errData = await response.json();
                throw new Error(errData.error || 'Failed to update savings entry');
            }

            // --- success ---
            if (onUpdate) {
                onUpdate();
            }
            if (onCancel) {
                onCancel();
            }

        } catch (err) {
            setError(err.message);
        }
    };
    // ========================

    // ---- JSX Return ----
    return (
        // Distinct color for Savings entries (same as the Add form)
        <form className="expense-form" onSubmit={onSubmit} style={{ backgroundColor: '#2E4057', border: '2px solid #4BC0C0' }}>
            <h3 style={{ color: '#4BC0C0' }}>Edit Savings ID: {savingsEntry.savings_id}</h3>

            {error && <div className="error-message">{error}</div>}

            <div className = "form-group">
                <input
                    type="number" 
                    name="amount"
                    value= {amount} 
                    onChange={onChange}
                    placeholder="Amount (ex: 250.00)"
                    required
                />
                <input
                    type="text"
                    name="goal" 
                    value={goal} 
                    onChange={onChange}
                    placeholder="Goal (ex: Vacation, Emergency)"
                    required
                />
            </div>

            <div className = "form-group">
                <DatePicker
                    selected={date} 
                    onChange={handleDateChange} 
                    dateFormat="yyyy/MM/dd"
                    placeholderText="Date"
                    required
                    className="custom-date-picker" 
                    id={`edit-savings-date-${savingsEntry.savings_id}`}
                />
                <input
                    type="text"
                    name="description"
                    value={description} 
                    onChange={onChange}
                    placeholder="Description (ex: Transfer to HYSA)"
                />
            </div>
            
            <div className="form-group" style={{ gap: '10px' }}>
                <button type="submit" style={{ flex: 2, background: '#4BC0C0' }}>Save Changes</button>
                <button 
                    type="button" 
                    onClick={onCancel} 
                    style={{ flex: 1, background: '#FF6384' }}
                >
                    Cancel
                </button>
            </div>
        </form>
    );
    // ====================
};

export default EditSavingsForm;